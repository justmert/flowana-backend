...
class GithubWidgets():
    ...
    def _score_pull_request_activity(self, owner, repo):
        try:
            ...
            ref2 = self.get_db_ref(owner, repo).document(
                'average_days_to_close_pull_request').get(field_paths=['data']).to_dict()
            ...
            if pull_requests:
                ...
                for pull_request in pull_requests:
                    days_since_created = (
                        now - parser.isoparse(pull_request['createdAt'])).days
                    comment_reward = pull_request['comment_count'] * \
                        comment_reward_factor
                    if pull_request['closed']:
                        days_since_closed = (
                            now - parser.isoparse(pull_request['closedAt'])).days
                        ...
                        if avg_days_to_close_pull_request is None:
                            ...
                            close_time_factor = avg_days_to_close_pull_request / \
                                (time_to_close +
                                 0.01)  # add a small constant to avoid division by zero
                        PRAS += (weight_closed * close_time_factor +
                                 comment_reward) * math.exp(-lambda_ * days_since_closed)
                    else:  # issue is still open
                        PRAS += (weight_open + comment_reward) * \
                            math.exp(-lambda_ * days_since_created)
    ...
    def _score_issue_activity(self, owner, repo):
        try:
            ...
            ref2 = self.get_db_ref(owner, repo).document(
                'average_days_to_close_issue').get(field_paths=['data']).to_dict()
            ...
            if issues:
                ...
                for issue in issues:
                    days_since_created = (
                        now - parser.isoparse(issue['createdAt'])).days
                    comment_reward = issue['comment_count'] * \
                        comment_reward_factor
                    if issue['closed']:
                        days_since_closed = (
                            now - parser.isoparse(issue['closedAt'])).days
                        time_to_close = days_since_created - days_since_closed
                        if avg_days_to_close_issue is None:
                            ...
                            close_time_factor = avg_days_to_close_issue / \
                                (time_to_close + 0.01)
                        IAS += (weight_closed * close_time_factor +
                                comment_reward) * math.exp(-lambda_ * days_since_closed)
                    else:  # issue is still open
                        IAS += (weight_open + comment_reward) * \
                            math.exp(-lambda_ * days_since_created)
    ...
    def _score_release_activity(self, owner, repo):
        try:
            ref = self.get_db_ref(owner, repo).document(
                'recent_releases').get(field_paths=['data']).to_dict()
    ...
    def contributors(self, owner, repo, **kwargs):
        ...
        for i in range(0, len(data), chunk_size):
            ...
            self.get_db_ref(owner, repo).document(doc_name).set({
                'data': chunk
            })
    ...
    def punch_card(self, owner, repo, **kwargs):
        ...
        self.get_db_ref(owner, repo).document('punch_card').set({
            'data': data
        })
