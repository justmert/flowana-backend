...
class DiscourseWidgets():
        ...
        while data:
            ...
            for topic in topics:
                created_date = datetime.strptime(
                    topic['created_at'], '%Y-%m-%dT%H:%M:%S.%fZ').date()
                created_date = created_date.strftime(
                    '%Y-%m-%d')  # format it to 'yyyy-mm-dd'
    ...
    def users(self, **kwargs):
        ...
        while data:
            data = self.actor.discourse_rest_make_request(
                f'/directory_items', variables={'order': 'likes_received', 'desc': 'true', 'period': 'all', 'page': page})
    ...
    def tags(self, **kwargs):
        ...
        if not self.is_valid(data):
            logger.info(
                f'[#invalid] No tags for protocol ')
            return
        ...
        self.collection_refs['discourse'].document(
            'tags').set({'data': sorted_tags})

    def top_topics(self, **kwargs):

        topics = {}
        for interval in ['all', 'yearly', 'quarterly', 'monthly', 'weekly', 'daily']:
            topics[interval] = []
            ...
            if not self.is_valid(data):
                logger.info(
                    f'[#invalid] No top topics for protocol ')
                continue
    ...
    def latest_topics(self, **kwargs):
        ...
        latest_topics = {}
        for order in ['default', 'created', 'activity', 'views', 'posts', 'likes', 'op_likes']:
            latest_topics[order] = []
            ...
            if not self.is_valid(data):
                logger.info(
                    f'[#invalid] No latest topics for protocol ')
                return
        ...
        self.collection_refs['discourse'].document(
            'latest_topics').set({'data': latest_topics})
    ...
    def latest_posts(self, **kwargs):
        ...
        if not self.is_valid(data):
            logger.info(
                f'[#invalid] No latest posts for protocol ')
            return
        ...
        self.collection_refs['discourse'].document(
            'latest_posts').set({'data': latest_posts})
    ...
    def top_users(self, **kwargs):
        ...
        for interval in ['all', 'yearly', 'quarterly', 'monthly', 'weekly', 'daily']:
            top_users[interval] = {}
            for order in ['likes_received', 'likes_given', 'topic_count', 'post_count', 'topic_entered', 'posts_read', 'days_visited']:
                top_users[interval][order] = []

                # formatting will be in frontend
                data = self.actor.discourse_rest_make_request(
                    f'/directory_items', variables={'order': order, 'desc': 'true', 'period': interval})
        ...
        self.collection_refs['discourse'].document(
            f'top_users').set({'data': top_users})
